...
import { getSimilarText } from "./task";
...
let userClickUpTasks: Array<{
  id: string;
  name: string;
  description?: string;
  url: string;
}> = []; // Cache tasks ของ user
...
async function processTaskMatching(
  userId: string,
  transcript: string,
  confidence: number
): Promise<void> {
  try {
    ...
    if (matchResult.matches.length > 0) {
      ...
      const goodMatches = matchResult.matches.filter(
        (match) => match.confidence === "high" || match.confidence === "medium"
      );
...
async function sendTaskMatchSuggestion(
  userId: string,
  transcript: string,
  matches: TaskMatch[]
): Promise<void> {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${user.displayName || user.username}** พูดว่า: "${transcript}"`
      )
      .addFields({
        name: "📋 Task ที่เข้าใจที่สุด",
        value: `**${topMatch.taskName}**\nความมั่นใจ: ${(
          topMatch.matchScore * 100
        ).toFixed(1)}% (${topMatch.confidence})\n[ดู Task ใน ClickUp](${
          topMatch.taskUrl
        })`,
        inline: false,
      })
...
• 🎯 **ความแม่นยำ** - แสดงความแม่นยำของแต่ละข้อความ`;
  ...
  if (message.content === "!train") {
    const getAllTask = await searchAllTeamTasks();    
    let userInputs:any[] = [];

    const allTaskName = getAllTask.forEach((task) => userInputs.push(task.name))
    console.log('all', getAllTask)
    const taskNames = ['dashboard', 'report', 'analytics', 'summary', 'Database Integration'];
    const response = await getSimilarText(userInputs, taskNames)
...
function isInvalidTranscript(transcript: string): boolean {
  ...
  if (words.length > 10) {
    ...
    for (const [word, count] of wordCount) {
      ...
      if (percentage > 0.5 && count > 10) {
        console.log(
          `⚠️ FILTERED: คำ "${word}" ซ้ำผิดปกติ: ${count}/${
            words.length
          } ครั้ง (${(percentage * 100).toFixed(1)}%)`
        );
        return true;
      }
    }
  ...
  if (trimmed.length > 50) {
    ...
    try {
      ...
      if (matches.length > 5) {
        console.log(
          `⚠️ FILTERED: Pattern "${firstPart}" ซ้ำผิดปกติ: ${matches.length} ครั้ง`
        );
        return true;
      }
    } catch (error) {
...
async function sendTranscriptToDiscord(
  ...
  try {
    ...
    console.log(
      `📤 ส่ง real-time transcript: "${transcript}" ${
        hasTaskMatch ? "(with ClickUp match)" : "(casual)"
      }`
    );
  } catch (error) {
...
export async function addSessionTranscript(
  ...
  if (isInvalidTranscript(transcript)) {
    console.log(
      `⚠️ ข้าม transcript ที่ผิดปกติ: "${transcript.substring(0, 50)}..."`
    );
...
async function processUserTaskMatching(
  email: string,
  transcripts: string[]
): Promise<void> {
  try {
    ...
    console.log(
      `📋 พบ ${tasks.length} tasks ในทีม สำหรับการ match กับ ${email}`
    );
...
async function sendUserTaskMatches(
  email: string,
  transcripts: string[],
  matches: TaskMatch[]
): Promise<void> {
  try {
    ...
    const discordUserId = Object.keys(userMapping).find(
      (id) => userMapping[id] === email
    );
    ...
    console.log(
      `📤 ส่ง Task Match สำหรับ ${userDisplayName}: ${topMatch.taskName}`
    );
  } catch (error) {
    ...
    if (action === "complete") {
      ...
      if (updatedTask) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Task **${updatedTask.name}** ได้รับการอัปเดทเป็น Complete โดย ${
      ...
      if (updatedTask) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Task **${updatedTask.name}** ได้รับการอัปเดทเป็น To Do โดย ${
              user.displayName || user.username
            }`
          )
          .setColor(0x0099ff)
